...
module.exports = {
  ...
  construct: function(self, options) {
    ...
    self.push = function(type, name, options, context) {
      ...
      if (exists) {
        self.pushed[self.assetTypes[type].key].push({ type: type, file: filePath, web: webPath, data: data, preshrunk: options.preshrunk, when: when, minify: self.apos.options.minify });
      }
      return exists;
    };
      ...
      return async.series({
        checkCache: function (callback) {
          ...
          return async.map(assets, function(asset, callback) {
            ...
            return self.apos.attachments.md5(asset.file, callback);
          }, function(err, md5s) {
            if (err) {
              return callback(err);
            }
    ...
    self.pushDefaults = function() {
      ...
      for (i in self.stylesheets) {
        self.pushAsset('stylesheet', self.stylesheets[i].name, self.stylesheets[i]);
      }
      for (i in self.scripts) {
        self.pushAsset('script', self.scripts[i].name, self.scripts[i]);
      }
      for (i in self.templates) {
        self.pushAsset('template', self.templates[i].name, self.templates[i]);
      }
    };
    ...
    self.addHelpers({
      ...
      scripts: function(when) {
        ...
        if (self.apos.options.minify) {
          ...
          result += '<script src="' + (self.prefix ? self.prefix : '') + '/apos-minified/' + when + '-' + self.generation + '.js"></script>\n';
