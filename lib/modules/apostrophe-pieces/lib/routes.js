...
module.exports = function(self, options) {
  ...
  self.routes.update = function(req, res) {
    ...
    return async.series({
      ...
      beforeSave: function(callback) {
        return self.beforeSave(req, req.piece, callback);
      },
  ...
  self.routes.publish = function(req, res) {
    // _id is provided
    var piece = req.body;
    // get the full piece and pass it to update()
    self.findIfContextual(req, piece, function(err) {
      piece.published = true;
      req.piece = piece;
      return self.routes.update(req, res);
    });
  };
  ...
  self.routes.editor = function(req, res) {
    return fs.exists(self.editorViews + '/editor.html', function(exists) {
      if (exists) {
        return res.send(self.render(req, 'editor/editor', {}));
      }
  ...
  self.routes.create = function(req, res) {
    return fs.exists(self.editorViews + '/create.html', function(exists) {
      if (exists) {
        return res.send(self.render(req, 'editor/create', {}));
      }
