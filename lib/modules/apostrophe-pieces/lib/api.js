...
module.exports = function(self, options) {
  ...
  self.editorViews = self.apos.rootDir + '/lib/modules/' + self.__meta.name + '/views/editor';

  self.generateScaffolding = function(callback) {
    var req = self.apos.tasks.getReq();
    var template = self.render(req, 'editor', { options: self.options, schema: self.schema });
    mkdirp(self.editorViews);
    return fs.writeFile(self.editorViews + '/editor.html', template, function(err) {
      if (err) {
          return console.log(err);
      }
      console.log('Saved a template or something!');
      return callback();
    });
  };
